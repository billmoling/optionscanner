"""Risk rule evaluation for the portfolio manager."""
...
class RiskEvaluator:
    ...
    def _check_liquidity(
        ...
        if min_open_interest is not None and "open_interest" in positions.columns:
            illiquid = positions[pd.to_numeric(positions["open_interest"], errors="coerce") < float(min_open_interest)]
        ...
        if max_spread is not None and "bid_ask_spread_pct" in positions.columns:
            wide = positions[pd.to_numeric(positions["bid_ask_spread_pct"], errors="coerce") > float(max_spread)]
